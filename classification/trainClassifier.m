function [trainedClassifier, validationAccuracy,rel_score] = trainClassifier(trainingData, response,class_names,class_type,varargin)
...
%     c2= varargin{5}; 
% 
%     timepoints=length(all_X); 
%     rel_score=nan(1,timepoints);
%     result=cell(1,timepoints); 
% 
% 
%     for i = 1:timepoints
%         curX=all_X{i}; 
...
%         curY=curY([c1,c2]);
% 
%         result{i}=trainedClassifier.predictFcn(curX); 
% 
%         rel_score(i)= sum(result{i}==curY')/length([c1,c2]); 
%     end
% 
%     actualY=all_Y{t};
%     actualY=actualY([c1,c2]); 
% 
%     base_score=sum(result{t}==actualY')/length(result{t}); 
% 
%     rel_score=rel_score-base_score;
%     rel_score(t)=NaN; 
% 
% end
