function [outcomes] = get_pop_sparseness_before(ds_events,ex_thresh,time_bin,before)
%%
...
for d=1:12
    ...
    [rand_resp,pv_resp,som_resp,som_onsets,pv_onsets,ex_resp,ex_type] = extract_onsets(cur_onsets,ds_events,d,ex_thresh);
    ...
    outcomes(d).sparseness_vect=sparseness_vect; 
...
for d=1:12
    ...
    for n=1:size(som_resp,1)
        ...
        for tr=1:size(som_resp,3)
            peak_diff=som_onsets(tr).peak-som_onsets(tr).onset;
            binned_resp=sum(som_resp(n,100-before:100-before+bin_size,tr),2);
        ...
        for tr=1:size(pv_resp,3)
            peak_diff=pv_onsets(tr).peak-pv_onsets(tr).onset;
            binned_resp=sum(pv_resp(n,100-before:100-before+bin_size,tr),2);
            if binned_resp>above_vect(n)
           ...
           for tr=1:size(ex_resp,3)
            ...
            binned_resp=sum(ex_resp(n,100-before:100-before+bin_size,tr),2);
            if binned_resp>above_vect(n)
                above_vect(2,n)=above_vect(2,n)+1;         
